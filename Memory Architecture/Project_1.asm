...
# added by acd : L1 miss counter
li $a0, 0 
	     
MAIN_1:	
	li $t0, 0x100100c0	# location of data in RAM
	li $t1, 45			# the wanted data is 45
	li $t2, 0
	li $t3, 100			# the search for the wanted data (5) iterates 100 times
	Loop:
		jal L1_Lookup
		addi $t2, $t2, 1
		bne $t2, $t3, Loop
		j END
	# The value of $s0 (no-hit-counter)  is 1 and the value of $s1 (hit-counter) is 99, so the miss rate of the L1 cache is 1%. 
...
L2_Lookup:
	addi $a0, $a0, 1			     # increment L1 miss counter (added by ACD)	                                                                                      
